...
const AGENT_LONG_RUNNING_FIRST_NOTIFY_MS = 3_600_000;  // 1 小时：主会话仍活跃时提醒
  ...
  private runIdleWatchdog(): void {
    ...
    for (const [chatId, session] of this.chatSessions) {
      ...
      if (
        ...
        const header = "任务还在运行";
        const parts = [
          `任务已经运行约 ${runningHours} 小时，目前还在工作中。`,
    ...
    for (const [sessionId, task] of this.runningTasks) {
      ...
      if (
        ...
        const parts = [`「${task.description}」已经运行约 ${runningHours} 小时，目前还在工作中。`];
