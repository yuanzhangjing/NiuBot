...
describe("SYSTEM_RULES", () => {
  it("contains the engine-owned rules and recovery entry point", () => {
    ...
    expect(SYSTEM_RULES).toContain("使用已验证能满足所需生命周期的持久会话机制");
    expect(SYSTEM_RULES).toContain("工具原生持久会话经验证满足要求时也可使用");
    ...
    expect(SYSTEM_RULES).toContain("复杂或多步骤本身不自动创建 Goal");
    expect(SYSTEM_RULES).toContain("只有用户明确要求启动 Goal 时才创建");
    expect(SYSTEM_RULES).not.toContain("只有用户明确要求 Goal 或持续跟踪时才进入 Goal");
    expect(SYSTEM_RULES).toContain("NiuBot 管理的消息、联系人、记忆、会话和任务元数据必须通过 nbt CLI 访问");
    expect(SYSTEM_RULES).not.toContain("防护已移除");
