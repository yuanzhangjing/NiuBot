...
function createSeed(root: string): string {
...
describe("runtime launcher", () => {
  ...
  it("moves a legacy v1 home to the shared seed while preserving rollback", async () => {
    const root = temporaryRoot();
    const home = path.join(root, "home");
    ...
    expect(state.current).toMatchObject({ storage: "shared" });
    expect(state.previous).toMatchObject({ storage: "legacy", runtimePath: fs.realpathSync.native(legacy) });
