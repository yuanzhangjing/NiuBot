...
- 只读任务（调研/审查/分析）随时可用；写任务（开发，developer 角色）直接在目标仓库内修改（git 操作由 Worker 按任务指引自行执行，base 提交/分支写进任务内容）。
...
```bash
nbt worker job create --work <work-id> --worker <profile> --file /tmp/job.md [--workdir <dir>]
```

- `--worker`：`general` / `researcher` / `reviewer`（只读），`developer`（写，直接在目标仓库内修改；git 操作由 Worker 自行执行，base 提交/分支约束写进 job 内容）；
- `/tmp/job.md` 内容 = 明确任务 + 完成标准 + 必要上下文（自由 Markdown）；
- `--workdir`：目标目录（**developer 写任务必须显式指定**，即目标仓库路径）；
- 输出 Job ID（`job_...`）。
